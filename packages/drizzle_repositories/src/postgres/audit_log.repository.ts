...
> extends PgDrizzleRepository<TSchema, TSchema["auditLogs"]> {
  ...
  async getByEntity(entityType: AuditLogEntityType, entityId: string, options?: { limit?: number; offset?: number }) {
    ...
    const t = this.t as unknown as Record<string, PgColumn>
  ...
  async getByDateRange(startDate: number, endDate: number, options?: { limit?: number; offset?: number }) {
    ...
    const t = this.t as unknown as Record<string, PgColumn>
    return this.findMany({
      where: and(gte(t.created_at, startDate), lte(t.created_at, endDate)),
      pagination: { limit, page: Math.floor(offset / limit) + 1 },
      orderBy: [{ column: t.created_at, direction: "desc" as const }],
    })
  }
  ...
  async getByParent(parentId: string, options?: {
    ...
    const countResult = await this.db.select({ count: sql<number>`count(*)` }).from(this.t as never).where(whereClause)
    ...
    } as Record<string, PgColumn>).from(this.t as never).where(whereClause)
      .orderBy(desc(t.created_at)).limit(limit).offset(offsetRows)
  ...
  async getById(id: string, workspaceId: string): Promise<AuditLogWithEmail | null> {
    const t = this.t as unknown as Record<string, PgColumn>
    const result = await this.db.select().from(this.t as never)
